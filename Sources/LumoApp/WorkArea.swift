    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: LumoDesign.gap) {
                PreviewPane(caption: T("原图"), image: state.previewBefore, busy: state.previewBusy)
                Image(systemName: "arrow.right")
                    .font(.system(size: 16, weight: .bold))
                    .foregroundColor(LumoDesign.accent)
                PreviewPane(caption: T("处理后"), image: state.previewAfter, busy: state.previewBusy)
            }
            .padding(.horizontal, LumoDesign.padWindow)
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: LumoDesign.gapTight) {
            HStack(spacing: LumoDesign.gapTight) {
                Text(T("效果预览")).font(LumoDesign.font(13, weight: .semibold))
                    .foregroundColor(LumoDesign.text)
                if state.previewBusy {
                    ProgressView().controlSize(.mini)
                }
                Spacer()
                LumoButton(title: state.previewAfter == nil ? T("生成预览") : T("刷新预览"),
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: LumoDesign.gapGroup) {
                ...
                InspectorGroup(T("扫描增强"), hint: T("选一种效果，左边的预览会按这一页实时算")) {
                    VStack(spacing: 6) {
                ...
                // 微调：只在少数文件上才有用，默认折叠
                InspectorGroup(T("微调"), hint: T("只在少数文件上才需要")) {
                    VStack(spacing: 6) {
                        FilterRow(icon: "scissors", title: T("自动裁边"), desc: T("裁到纸张边缘，校正透视"),
                                  isOn: Binding(get: { state.filters.autoCrop ?? false },
                                                set: { state.filters.autoCrop = $0 }))
                        FilterRow(icon: "angle", title: T("纠偏"), desc: T("自动旋转回正"),
                                  isOn: Binding(get: { state.filters.deskew ?? false },
                                                set: { state.filters.deskew = $0 }))
                        FilterRow(icon: "squareshape.split.2x2", title: T("去网纹"), desc: T("消去印刷网点"),
                                  isOn: Binding(get: { state.filters.descreen ?? false },
                                                set: { state.filters.descreen = $0 }))
                        FilterRow(icon: "wand.and.sparkles", title: T("文本锐化"), desc: T("非锐化掩膜"),
                ...
                // 作用范围
                InspectorGroup(T("作用范围")) {
                    Picker("", selection: $state.pageScope) {
                        Text(T("全部页面")).tag("all")
                        Text(T("当前页")).tag("current")
                        Text(T("指定范围")).tag("range")
                    }
                    .pickerStyle(.segmented)
                    .labelsHidden()
                    if state.pageScope == "range" {
                        TextField(T("例如 1-3,5"), text: $state.pageRange)
                ...
                // 压缩方案：三档卡片，预估体积直接给出来
                InspectorGroup(T("智能压缩"), hint: T("体积已预估")) {
                    VStack(spacing: 6) {
                ...
                DisclosureGroup(isExpanded: $showAdvanced) {
                    VStack(alignment: .leading, spacing: LumoDesign.gap) {
                        HStack(spacing: LumoDesign.gap) {
                            ...
                            Text(T("自适应压缩（按页选最省编码）"))
                                .font(LumoDesign.font(12.5))
                            Spacer()
                        }
                        labeledPicker(T("彩色 / 灰度编码"), selection: $state.colorEncoder,
                    ...
                    Text(T("高级选项")).font(LumoDesign.font(12.5, weight: .medium))
                        .foregroundColor(LumoDesign.muted)
                }

                // 结果卡：处理完就地显示，不跳页
                if state.stage == .result, let r = state.result {
                    InspectorGroup(T("处理完成")) {
                        VStack(alignment: .leading, spacing: LumoDesign.gapTight) {
                            HStack(spacing: LumoDesign.gap) {
                                metric(T("原始"), lumoBytes(r.inSize))
                                metric(T("输出"), lumoBytes(r.outSize))
                                metric(T("压缩"), String(format: "%.1f×",
                                                      Double(r.inSize) / max(1, Double(r.outSize))))
                            }
                            Text(T("OCR %@ · %@ 字 · %@s", r.ocrEngine ?? T("无"),
                                    r.ocrChars, String(format: "%.1f", r.elapsedSec)))
                                .font(LumoDesign.font(11.5)).foregroundColor(LumoDesign.muted)
...
struct ReportSection: View {
    ...
    var body: some View {
        if let r = state.report {
            let a = r.analysis
            InspectorGroup(T("体检报告"), hint: T("自动读出来的，不用你填")) {
                VStack(alignment: .leading, spacing: LumoDesign.gapTight) {
                    HStack(spacing: 6) {
                        metric(T("色彩"), a.colorModeText)
                        metric(T("分辨率"), "\(a.estimatedDpi)")
                        metric(T("倾斜"), String(format: "%.2f°", a.skewAngle))
                        metric(T("背景"), a.bgText)
                    }
                ...
                VStack(alignment: .leading, spacing: 1) {
                    HStack(spacing: 5) {
                        ...
                        if plan.recommended == true {
                            Text(T("推荐")).font(LumoDesign.font(9.5, weight: .bold))
...
struct BackgroundStrengthSlider: View {
    ...
    private var label: String {
        ...
        case ..<0.2: return T("保留纸张质感")
        case ..<0.45: return T("轻度清理")
        case ..<0.7: return T("干净")
        default: return T("很干净（接近纯白）")
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text(T("背景清理")).font(LumoDesign.font(10.5)).foregroundColor(LumoDesign.faint)
        }
        .padding(.horizontal, 10)
        .padding(.vertical, 8)
        .background(RoundedRectangle(cornerRadius: LumoDesign.radiusControl).fill(LumoDesign.panelAlt))
