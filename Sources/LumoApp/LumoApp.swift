...
final class AppState: ObservableObject {
    ...
    var activeFilters: [String] {
        ...
        if filters.autoCrop == true { s.append("自动裁边") }
        if filters.deskew == true { s.append("纠偏") }
        if filters.descreen == true { s.append("去网纹") }
        if (filters.sharpen ?? 0) > 0 { s.append("文本锐化") }
        return s
    }
...
extension AppState {
    ...
    private func save(_ r: ProcessResponse) async {
        ...
        guard FileManager.default.fileExists(atPath: src.path) else {
            errorMessage = "产物文件已经不在临时目录里了，请重新处理一次。"
            return
        }
