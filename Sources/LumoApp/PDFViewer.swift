...
struct PDFViewerSheet: View {
    ...
    private var header: some View {
        HStack(spacing: LumoDesign.gap) {
            ...
            LumoButton(title: "在预览中打开", icon: "arrow.up.forward.app", kind: .secondary) {
                // 逃生出口：内置阅读器是"够用就好"，真要认真读还是系统预览更顺手。
                NSWorkspace.shared.open(doc.url)
            }
            LumoCircleButton(icon: "xmark", help: "关闭阅读器") { onClose() }
        }
    ...
    private var footer: some View {
        HStack(spacing: LumoDesign.gap) {
            ...
            LumoCircleButton(icon: "minus.magnifyingglass", help: "缩小") { handle.zoom(1 / 1.25) }
            LumoCircleButton(icon: handle.isFitWidth ? "arrow.up.left.and.arrow.down.right" : "1.magnifyingglass",
                             help: handle.isFitWidth ? "当前：适应宽度" : "当前：实际大小") {
                handle.setFitWidth(!handle.isFitWidth)
            }
            LumoCircleButton(icon: "plus.magnifyingglass", help: "放大") { handle.zoom(1.25) }
        }
        .padding(.horizontal, LumoDesign.padWindow)
        .frame(height: 48)
        .background(glass.barMaterial)
