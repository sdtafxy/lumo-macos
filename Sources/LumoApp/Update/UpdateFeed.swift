...
enum UpdateError: LocalizedError, Equatable, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .badRepository(let r):
            return "更新源地址不合法：\(r)（应形如 owner/repo）"
        case .repositoryInaccessible(let r):
            ...
            //   不存在 = 访问不了（私有/写错/被删）。
            return "更新源 \(r) 不可访问（多半是私有仓库，或仓库名写错了）。"
                 + "自动更新需要「公开」的仓库——私有的仓库不允许匿名读取，"
                 + "客户端拿不到发布信息。"
        case .rateLimited:
            return "访问 GitHub 太频繁被限流了。过一会儿再试，或到 Releases 页面手动下载。"
        case .httpStatus(let c):
            return "更新源返回 HTTP \(c)"
        case .malformedResponse:
            return "更新源的返回看不懂（可能接口变了）"
        case .noArchiveInRelease(let v):
            return "\(v) 这个发布里没有可用的安装包（.zip）"
        case .missingSignature:
            return "这个包没有签名文件，已拒绝安装（配置了公钥就必须验签，不会降级成只查哈希）"
        case .signatureMismatch:
            return "签名校验失败——包的内容和签名对不上，已拒绝安装"
        case .checksumMismatch(let e, let a):
            return "校验和不符（期望 \(e.prefix(12))…，实际 \(a.prefix(12))…），包可能在传输中损坏"
        case .noVerificationData:
            return "这个发布没有任何校验信息，已拒绝安装"
        case .unpackFailed(let m):
            return "解压失败：\(m)"
        case .badBundle(let m):
            return "解出来的包不对：\(m)"
        case .versionMismatch(let e, let f):
            return "包里的版本是 \(f)，与预期的 \(e) 不符"
        case .targetNotWritable(let p):
            return "没有权限替换 \(p)。把 App 拖到「应用程序」文件夹，或手动下载新版安装。"
        }
    }
