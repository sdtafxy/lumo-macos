...
enum UpdateInstaller {
    ...
    static func unzip(_ archive: URL, into dir: URL) throws -> String {
        ...
        if p.terminationStatus != 0 {
            let msg = String(data: errPipe.fileHandleForReading.readDataToEndOfFile(),
                             encoding: .utf8) ?? ""
            throw UpdateError.unpackFailed(msg.isEmpty ? T("ditto 退出码 %@", p.terminationStatus) : msg)
        }
        return ""
    }
    ...
    static func findApp(in dir: URL) throws -> URL {
        ...
        throw UpdateError.badBundle(T("解压结果里没有 .app"))
    }
    ...
    static func validateBundle(_ app: URL,
        ...
        else { throw UpdateError.badBundle(T("解出来的东西不是 macOS App 包（没有 Info.plist）")) }

        if let id = plistValue(app, "CFBundleIdentifier"), id != expectedBundleID {
            throw UpdateError.badBundle(T("包标识是 %@，不是 %@", id, expectedBundleID))
        }
        guard let v = plistValue(app, "CFBundleShortVersionString") else {
            throw UpdateError.badBundle(T("包里没有版本号"))
        }
        guard let got = SemanticVersion(v), let want = SemanticVersion(expectedVersion) else {
            throw UpdateError.badBundle(T("版本号无法识别（包里 %@，期望 %@）", v, expectedVersion))
        }
        guard got == want else {
    ...
    static func writeHelper(pid: Int32, target: URL, staged: URL, workDir: URL) throws -> URL {
        ...
        // ★ 启动之前先让 shell 自己校验一遍语法。
        //
        // 这条守卫是被一次真实故障逼出来的：helper 脚本是用字符串拼出来的，
        // 里面混进一个多余的字符就会整体语法错误——而那行错误会打在
        // **已经被 App 摘掉的 stderr** 上（见 launchHelper 的注释）。
        // 结果就是"点了更新，什么都没发生"，日志里也一片空白。
        // `sh -n` 只解析不执行，几毫秒的代价，换来的是**坏脚本当场报错**：
        // 那时 App 还没退出，用户还来得及重试。
        if let problem = shellSyntaxProblem(script) {
            throw UpdateError.helperScriptInvalid(problem)
        }
        return script
    }

    /// 用 `/bin/sh -n` 校验脚本语法，返回问题描述（nil = 没问题）。
    ///
    /// 为什么值得单独跑一个进程：更新助手是整条链路上唯一"以脚本形式落盘再执行"的东西，
    /// 而它的输入是拼出来的字符串。别的地方都有类型检查兜着，这里没有。
    static func shellSyntaxProblem(_ script: URL) -> String? {
        let p = Process()
        ...
        do {
            ...
            return "无法执行 sh -n：\(error.localizedDescription)"
        }
        p.waitUntilExit()
        ...
        return text.isEmpty ? "sh -n 退出码 \(p.terminationStatus)" : text
    }
