...
/// 这次安装**实际**验到了什么级别。
///
/// 为什么要把它当成一等公民报给用户：手册 §4 的核心认知是
/// **SHA-256 是「完整性」检查，不是「来源」检查**——
/// 它和包放在同一个 Release 里，能换包的人就能换哈希。
/// 只有内置公钥的验签才真正回答"这是不是我发的"。
/// 把级别如实显示出来，比含糊地说一句"已校验"要诚实得多。
enum UpdateSecurityLevel: String, Equatable, Sendable {
    ...
    var label: String {
        ...
        case .checksum:   return "SHA-256 完整性"
        case .signature:  return "Ed25519 签名"
        }
    }
    ...
    var explanation: String {
        switch self {
        case .httpsOnly:
            return "只保证了传输过程没被改，没有校验包的来源。"
        case .checksum:
            return "校验了包没在传输中损坏。注意：校验和与包放在同一处，能换包的人也能换校验和——它证明不了「这是官方发的」。"
        case .signature:
            return "用 App 内置的公钥验了签名，可以证明这个包是持有私钥的人发布的。"
        }
    }
}
