...
struct RootView: View {
    ...
    /// 订阅语言状态：它一变，body 就重跑，`.id()` 也随之变化 → 内容整棵重建。
    ...
    var body: some View {
        windowChrome
            ...
            .alert("Lumo", isPresented: Binding(get: { state.errorMessage != nil },
                ...
                Button(T("好")) { state.errorMessage = nil }
            } message: {
    ...
    private var windowChrome: some View {
        ZStack {
            ...
            // ★ 语言变了就把内容整棵重建。
            //
            // 为什么要用 `.id()` 这种"重"手段，而不是让每个 View 各自订阅环境：
            // 手动订阅时**漏掉一个 View 不会报错**，表现为"切了英文但某处还是中文"——
            // 这种瑕疵要逐屏找。整棵重建虽然丢一点瞬时状态（悬停、滚动位置），
            // 但保证没有漏网的。语言是极少改的设置，这点代价完全可以接受。
            //
            // 注意 `.id()` 只挂在这一层：AppState（当前文件、预览、结果）
            // 由上面的 WindowState 持有，不在这个子树里，所以**不会**被重建掉。
            .id(lang.resolved)
            // ★ 让顶栏真的贴到窗口顶边。
            //
    ...
    private var eventSink: LumoEventSink {
        LumoEventSink(
            ...
            presentFailed: { name in
                state.errorMessage = T("没找到内置的「%@」（安装包可能不完整）", name)
            },
    ...
    private func handlePicked(_ res: Result<URL, Error>) {
        ...
        case .failure(let err):
            state.errorMessage = T("没能打开这个文件：%@", err.localizedDescription)
        }
    }
    ...
    private func pasteFromClipboard() {
        guard let url = FileDrop.fromPasteboard() else {
            state.errorMessage = T("剪贴板里没有 PDF。可以复制一个 PDF 文件，或复制它的完整路径再试。")
            return
        }
        Task { @MainActor in await state.load(url: url, backend: backend) }
    }
    ...
    func body(content: Content) -> some View {
        content
            ...
            .onReceive(NotificationCenter.default.publisher(for: .lumoPresentDocFailed)) { note in
                sink.presentFailed((note.object as? String) ?? T("内置文档"))
            }
...
enum BundledDoc {
    // ⚠️ 这两个是**打包时的文件名**（Resources 里就是这两个 .pdf），
    //    **绝不能翻译、也不要拿 T() 包**——翻了就找不到文件，
    //    英文界面下会一律报"内置文档缺失"。显示用的名字见 title(_:)。
    static let sample = "示例扫描件"
    static let design = "Lumo-设计文档"

    /// 给人看的名字。与文件名解耦，所以这里翻译是安全的。
    static func title(_ name: String) -> String {
        switch name {
        case sample: return T("示例扫描件")
        case design: return T("设计文档")
        default:     return name
        }
    }
    ...
    static func ref(_ name: String, url: URL) -> PDFDocRef {
        PDFDocRef.file(url, captionPrefix: T("随 App 打包"))
    }
