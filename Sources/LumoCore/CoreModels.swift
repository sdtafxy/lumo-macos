...
public enum EnhancePreset: String, CaseIterable, Codable, Sendable {
    ...
    public var title: String {
        switch self {
        case .auto:     return T("自动增强")
        case .bw:       return T("黑白")
        case .color:    return T("增强")
        case .contrast: return T("对比度")
        case .original: return T("原图")
        }
    }
    ...
    public var desc: String {
        switch self {
        case .auto:     return T("逐页判断：文字页走黑白，含图页保色彩")
        case .bw:       return T("只留纯黑与纯白，底灰、阴影、污渍全部清掉")
        case .color:    return T("保留彩色与插图，清背景、校正光照、略提饱和")
        case .contrast: return T("仍保持原有色调，只把发灰、太淡的画面拉开反差")
        case .original: return T("不做画面处理，只做识别与压缩")
        }
    }
...
extension Analysis {
    public var colorModeText: String {
        ["color": T("彩色"), "gray": T("灰阶"), "mono": T("单色")][colorMode] ?? colorMode
    }
    public var noiseText: String {
        noiseLevel < 0.22 ? T("低") : (noiseLevel < 0.5 ? T("中") : T("高"))
    }
    /// 背景明暗是否均匀。「不匀」才是该做背景去除的信号
    public var bgText: String { bgUnevenness > 0.12 ? T("不匀") : T("均匀") }
}
