...
public enum Workflow {
    public static func recommend(_ a: Analysis) -> Recommendation {
        ...
        if hasText {
            ...
            notes.append("已检测到文字层 → 跳过 OCR，避免重复识别与伪影")
        } else {
            ocr = OCRSpec(enabled: true, lang: "chi_sim+eng", output: "searchable", skip: false)
            notes.append("未发现文字层 → 建议 OCR 生成可搜索 PDF")
        }
        ...
        switch preset {
        case .color:
            notes.append("彩色文档 → 增强（保住图表与印章，不做二值化）")
        case .bw:
            notes.append("单色文档 → 黑白（局部自适应二值化，文字最锐利、体积最小）")
        default:
            notes.append("灰阶文档 → 自动增强（逐页判断：文字页走黑白，含图页保层次）")
        }
        if skew > 1.0 { notes.append(String(format: "检测到倾斜 %.1f° → 启用纠偏", skew)) }
        if needBG {
            notes.append(String(format: "背景明暗不匀 %.0f%% → 启用背景去除", uneven * 100))
        }
        if noise > 0.5 { notes.append("疑似半调网纹 → 启用去网纹") }
        notes.append("轻度锐化以还原文字边缘")
        ...
        case "color":
            ...
            notes.append("彩色文档 → 自适应压缩（文字页转单色 CCITT，插图页走 JPEG2000）")
        case "gray":
            compress = CompressSpec(adaptive: true, colorMode: "auto",
                                    colorEncoder: "jpeg", monoEncoder: "ccitt", quality: 70)
            notes.append("灰阶文档 → 自适应压缩（JPEG / CCITT）")
        default:
            compress = CompressSpec(adaptive: false, colorMode: "mono",
                                    colorEncoder: "jpeg", monoEncoder: "ccitt", quality: 70)
            notes.append("单色文档 → CCITT 组4 极限压缩")
        }
