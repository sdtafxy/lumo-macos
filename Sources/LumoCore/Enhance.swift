...
public enum Enhance {
    ...
    public static func autoCrop(_ image: CGImage, quad q: DocumentQuad) -> CropOutcome {
        ...
        guard isConvexQuad(tl, tr, br, bl) else {
            return kept("裁边：检出的四边形畸变，已跳过（保持原样）")
        }

        // 闸门 2：占比。≥0.96 说明检出的是整幅画布，裁了也没什么可裁；
        // <0.10 则多半是把页面上某个小方框（印章、表格、插图）当成了纸。
        if q.coverage >= 0.96 {
            return kept(String(format: "裁边：页面已铺满整幅（占比 %.0f%%），无需裁切", q.coverage * 100))
        }
        if q.coverage < 0.10 {
            return kept(String(format: "裁边：检出区域只占整幅 %.0f%%，疑似误检，已跳过", q.coverage * 100))
        }
    ...
    public static func applyReporting(_ image: CGImage, spec: EnhanceSpec,
        ...
        if judgedClean {
            notes.append("这一页体检判定为已经够干净：背景强度仍按设定应用，但视觉上可能看不出变化")
        }
