...
public enum PipelineError: LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .cannotOpen(let p):     return T("无法打开 PDF：%@", p)
        case .emptyPageSelection:    return T("页码范围没有选中任何页面")
        case .nothingRendered:       return T("页面渲染失败，文件可能已损坏")
        }
    }
}
...
public enum Pipeline {
    ...
    public static func process(fileURL: URL, spec: ProcessSpec, outURL: URL,
        ...
        for (k, i) in idxs.enumerated() {
            ...
            progress?(Double(k + 1) / Double(idxs.count), T("处理中 %@/%@ 页", k + 1, idxs.count))
        }

        // 干净页提示汇总成一条：逐页各发一条的话，几十页的文件会刷出几十行，
        // 那种"提示"只会被当成噪音，而它本来是要解释"为什么滑块看起来没反应"。
        if !cleanPages.isEmpty {
            notes.append(T("第 %@ 页体检判定为已经够干净：背景强度照常按设定应用，但这几页视觉上可能看不出变化", Self.pageList(cleanPages)))
        }
    ...
    private static func summarize(_ spec: ProcessSpec, _ n: Int, _ mrcPages: Int = 0) -> [String] {
        ...
        if let p = spec.enhance.preset, p != "original" {
            s.append(T("增强模式：%@", EnhancePreset(p).title))
        }
        if spec.enhance.autoCrop == true { s.append(T("自动裁边 + 透视校正")) }
        if spec.enhance.deskew == true { s.append(T("纠偏")) }
        if spec.enhance.bgRemove == true { s.append(T("背景去除")) }
        if spec.enhance.descreen == true { s.append(T("去网纹")) }
        if let a = spec.enhance.sharpen, a > 0 { s.append(T("文本锐化")) }
        ...
        if spec.ocr.enabled && !spec.ocr.skip {
            s.append(T("OCR 识别（%@）", OCR.name(of: spec.ocr.lang)))
        } else {
            s.append(T("跳过 OCR"))
        }
        let mode = spec.compress.adaptive ? T("自适应") : spec.compress.colorMode
        s.append(T("压缩：%@ / %@+%@ / 质量%@", mode, spec.compress.colorEncoder, spec.compress.monoEncoder, spec.compress.quality))
        if mrcPages > 0 { s.append(T("%@ 页启用分层（MRC）：文字 1bit 蒙版 + 低分辨率背景", mrcPages)) }
        return s
    }
