...
# ⓪ 先造一个**最小的 LumoCore 模块**。
#
# 更新器从前是"零依赖、能单独编"的；加了界面语言之后它要调 Core 的 `T()`，
# 于是 `import LumoCore` 在裸编译下会报 no such module。
# 与其为了测试把依赖拆掉，不如在这里照实造一个模块——只编 Localization.swift，
# 它是纯 Foundation 的，编译不到一秒。
MOD_DIR="$(mktemp -d)"
swiftc -swift-version 5 -parse-as-library -emit-module -emit-object \
  -module-name LumoCore \
  -emit-module-path "$MOD_DIR/LumoCore.swiftmodule" \
  Sources/LumoCore/Localization.swift \
  -o "$MOD_DIR/LumoCore.o"
ar rcs "$MOD_DIR/libLumoCore.a" "$MOD_DIR/LumoCore.o"
echo "--- 已造出最小 LumoCore 模块（Localization.swift）---"
